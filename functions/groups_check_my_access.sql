DROP FUNCTION IF EXISTS groups_check_my_access;

/* go up the heirarchy to see if this user is an admin at this level or any higher level */
CREATE OR REPLACE FUNCTION groups_check_my_access(target uuid, access_level TEXT)
RETURNS boolean as
$$
...
select (select count(*) FROM hierarchy as h 
  ...
  ON g.group_id = h.id AND g.user_id = auth.uid()
